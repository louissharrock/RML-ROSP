...
## RML_ROSP ##
if(TRUE){
  RML_ROSP_plot1 = RML_ROSP(spde_sim = spde_sim, n = n, n_obs = n_obs, t = t, 
...
## Save ##
setwd(fig_wd)
saveRDS(RML_ROSP_plot1,"RML_ROSP_plot1")
...
if(TRUE){
  ...
  ## RML Print Iteration Frequency ##
  if(TRUE){
    RML_print_iter_freq = NULL
  }
  
  ## ROSP Print Iteration Frequency ##
  if(TRUE){
    ROSP_print_iter_freq = NULL
  }
  
  ## RML_ROSP Print Iteration Frequency ##
  if(TRUE){
...
## RML_ROSP ##
if(TRUE){
  RML_ROSP_plot1 = RML_ROSP_v2(spde_sim = spde_sim, n = n, n_obs = n_obs, t = t, 
                            t_RML = t_RML, t_ROSP = t_ROSP, K = K, 
                            n_param = n_param, param_obs0 = param_obs0,
                            param_sig0 = param_sig0, param_bias0 = param_bias0,
                            log_scale = log_scale, log_indices = log_indices, 
...
if(TRUE){
  setwd(fig_wd)
  #pdf("RML_plot1.pdf",width=7,height=7)
  tikz('RML_plot1.tex', standAlone=TRUE, height = 7, width = 7)
  par(mar=c(3,4.3,.6,2),mgp=c(2.7,1,0),family="LM Roman 10",cex=2)
  par(mfrow=c(3,3))
  plot_points = seq(1,t,100)
  if(FALSE){
  setwd("/Users/ls616/Google Drive/MPE CDT/PhD/Year 2/Application_Paper/Current/Draft5/Figures/Section4/RML_ROSP_plot1")
  RML_ROSP_plot1 = readRDS("RML_ROSP_plot1")
  }
  param_est = RML_ROSP_plot1$param_est
  ...
  dev.off()
  tools::texi2dvi('RML_plot1.tex',pdf=T)
}
...
if(TRUE){
  ...
  setwd(fig_wd)
  #pdf("ROSP_plot1.pdf",width=7,height=7)
  tikz('ROSP_plot1.tex', standAlone=TRUE, height = 7, width = 7)
  ...
  for(k in 1:n_obs){
    plot(plot_points,obs[[k]][plot_points,1],cex=.4,ylim=c(0,n),
         ...
         main=paste("Sensor",k),cex.axis=2.0,cex.lab=2.0,cex.main=2.0)
  ...
  dev.off()
  tools::texi2dvi('ROSP_plot1.tex',pdf=T)
}
...
if(TRUE){
  ...
  ## RML Plot Filename ##
  if(TRUE){
    RML_plot_filename = "RML_no_ROSP_plot1.pdf"
  }
...
## RML_ROSP ##
if(TRUE){
  RML_no_ROSP_plot1 = RML_ROSP(spde_sim = spde_sim, n = n, n_obs = n_obs, t = t, 
                           t_RML = t_RML, t_ROSP = t_ROSP, K = K, 
                           n_param = n_param, param_obs0 = param_obs0,
                           param_sig0 = param_sig0, param_bias0 = param_bias0,
                           log_scale = log_scale, log_indices = log_indices, 
...
## Save ##
setwd(fig_wd)
saveRDS(RML_no_ROSP_plot1,"RML_no_ROSP_plot1")
...
## RML_ROSP ##
if(TRUE){
  ROSP_no_RML_plot1 = RML_ROSP(spde_sim = spde_sim, n = n, n_obs = n_obs, t = t, 
                           t_RML = t_RML, t_ROSP = t_ROSP, K = K, 
                           n_param = n_param, param_obs0 = param_obs0,
                           param_sig0 = param_sig0, param_bias0 = param_bias0,
                           log_scale = log_scale, log_indices = log_indices, 
...
## Save ##
setwd(fig_wd)
saveRDS(ROSP_no_RML_plot1,"ROSP_no_RML_plot1")
...
target_loc = lapply(1:n_obs,function(j) matrix(W_coords[[j]],nrow=t,ncol=2,byrow=T))
...
if(TRUE){
  setwd(fig_wd)
  RML_ROSP_plot1 = readRDS("RML_ROSP_plot1")
  RML_no_ROSP_plot1 = readRDS("RML_no_ROSP_plot1")
  ROSP_no_RML_plot1 = readRDS("ROSP_no_RML_plot1")
}
  
for (i in 1:t){
  mean_square_errors_RML_ROSP[i] = sum((as.vector(fft_matrix%*%RML_ROSP_plot1$kf_means[i,])
                               -spde_sim_all$x[i,])^2)
  mean_square_errors_RML_no_ROSP[i] = sum((as.vector(fft_matrix%*%RML_no_ROSP_plot1$kf_means[i,])
                                        -spde_sim_all$x[i,])^2)
  mean_square_errors_ROSP_no_RML[i] = sum((as.vector(fft_matrix%*%ROSP_no_RML_plot1$kf_means[i,])
                                        -spde_sim_all$x[i,])^2)
...
library("plotrix")
setwd(fig_wd)
pdf("MSE_plot1.pdf",width=13,height=4)
...
ablineclip(h = mean(mean_square_errors_true_param_target_loc), x2 = 100000,lty=2, lwd = 2, col="blue")
ablineclip(h = mean(mean_square_errors_true_param_init_loc), x2 = 100000, lty=2,col="red", lwd = 2)
...
setwd(fig_wd)
cairo_pdf("KF_plot1_all_time_mult_coeff.pdf",width=13,height=9)
...
for(i in alpha_to_plot){
  ...
  axis(side = 1,at=xtick)
  lines(seq(kf_plot_start,kf_plot_end,kf_plot_freq),RML_ROSP_plot1$kf_means[seq(kf_plot_start,kf_plot_end,kf_plot_freq),i],col="red")
}
dev.off()

## All Time (one fourier coefficient) ##
setwd(fig_wd)
cairo_pdf("KF_plot1_all_time_one_coeff.pdf",width=13,height=4)
...
axis(side = 1,at=xtick)
lines(seq(kf_plot_start,kf_plot_end,kf_plot_freq),RML_ROSP_plot1$kf_means[seq(kf_plot_start,kf_plot_end,kf_plot_freq),alpha_to_plot_new],col="red")
dev.off()

## Start (one fourier coefficient) ##
setwd(fig_wd)
cairo_pdf("KF_plot1_start_one_coeff.pdf",width=6.5,height=4)
...
lines(seq(kf_plot2_start,kf_plot2_end,kf_plot2_freq),RML_ROSP_plot1$kf_means[seq(kf_plot2_start,kf_plot2_end,kf_plot2_freq),alpha_to_plot2],col="red")
dev.off()

## End (one fourier coefficient) ##
setwd(fig_wd)
cairo_pdf("KF_plot1_end_one_coeff.pdf",width=6.5,height=4)
par(mfrow=c(1,1),mar=c(2.2,1.5,.5,.6),mgp =c(1.2,.5,0),xpd=FALSE,family="LM Roman 10",cex=1.5)
...
lines(seq(kf_plot3_start,kf_plot3_end,kf_plot3_freq),RML_ROSP_plot1$kf_means[seq(kf_plot3_start,kf_plot3_end,kf_plot3_freq),alpha_to_plot3],col="red")
dev.off()


## Start & End (one fourier coefficient) ##
setwd(fig_wd)
cairo_pdf("KF_plot1_start_end_one_coeff.pdf",width=13,height=4)
